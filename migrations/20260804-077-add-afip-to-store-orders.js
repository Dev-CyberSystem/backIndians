...
/** Facturación electrónica ARCA (portado desde `integracionarca`, 053→077). */
...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    if (!table.afip_status) {
      for (const [col, def] of AFIP_COLS(Sequelize)) {
        await queryInterface.addColumn('store_orders', col, def);
      }
    }
  },
  ...
  async down(queryInterface) {
    ...
    if (table.afip_status) {
      for (const [col] of AFIP_COLS({})) {
        await queryInterface.removeColumn('store_orders', col);
      }
    }
  },
};
