...
## Actualización 2026-09-01 — RELEASE que junta 3 features — **mergeado a `master` local en ambos repos, sin pushear**

Las tres ramas están mergeadas a `master` (back +6, front +7 sobre `origin/master`), validadas local (typecheck + Jest store suites + `tsc -b`/Vitest 47/`npm run build`). **Falta**: `npm run release -- minor` (→ **v1.5.0**) para correr `test:full`, backup de prod, tag y deploy. Ninguna toca esquema de DB.
