...
- **Unificar el cobro de la tienda en MercadoPago**: decisión de negocio tomada el 2026-08-24 al adaptar el centro de ayuda, **ejecutada sólo en el contenido publicado**. El centro de ayuda (`/tienda/ayuda#pagos`) ya dice que el medio de pago es MercadoPago, pero `StoreCheckoutPage.tsx` sigue ofreciendo **efectivo y transferencia bancaria** y los T&C (`legal/TermsPage.tsx`, sección 8) también los mencionan. Hasta que se saquen del checkout, la ayuda y el checkout se contradicen frente al comprador. Falta además decidir qué pasa con los pedidos ya existentes pagados por transferencia, con `bank_transfer_*` en Settings y con la cuenta de caja `store_cash_account_id` (efectivo). Ver [02-FUNCTIONAL-MAP.md, sección 11c](02-FUNCTIONAL-MAP.md#11c-centro-de-ayuda-de-la-tienda).
