...
### BR-STORE-012 — El costo de envío depende de la zona (resto del país / Tucumán capital / Tucumán interior)
**Descripción**: para `shipping_type: 'delivery'` hay **tres tarifas**, en claves de `settings`: `shipping_cost` (resto del país, ya existía), `shipping_cost_tucuman_capital` (San Miguel de Tucumán) y `shipping_cost_tucuman_interior` (resto de la provincia de Tucumán). La provincia sale de `shipping_address.state` (un `<select>` de las 24 jurisdicciones AR en el checkout); la distinción capital/interior sale de `shipping_address.shipping_zone`, un selector explícito que **solo aparece si la provincia es Tucumán** (la ciudad es texto libre y no alcanza para saberlo). `resolveShippingZone()` deriva la zona: no-Tucumán → `national`; Tucumán sin selector → `tucuman_interior` (el más caro, para no subfacturar). `getShippingCostForZone()` cae a `shipping_cost` si la clave de la zona de Tucumán está vacía o no es numérica — nunca cobra 0 por omisión. El **umbral de envío gratis** (`free_shipping_min`) es **global**, igual para las tres zonas. La zona elegida se guarda en `shipping_address.shipping_zone` (JSON, sin migración) y se muestra en el detalle del pedido en el panel.
**Módulo**: Tienda (10) / Configuración (14).
**Fuente**: `backIndians/src/services/store.service.ts` (`resolveShippingZone`, `getShippingCostForZone`, `computeOrderTotals`), `settings.service.ts` (claves nuevas en `VALID_KEYS` y `PUBLIC_SETTING_KEYS`), `frontIndians/src/pages/store/StoreCheckoutPage.tsx` + `src/data/argentinaProvinces.ts`. Rama `feature/checkout-envio-por-zona`. Tests: `src/__tests__/api/checkout-quote.test.ts`.
...
**Estado**: Vigente (rama sin mergear al 2026-09-01).
