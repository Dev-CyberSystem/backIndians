...
3. Por eso existe **`src/config/ensureSchema.ts`**, llamado siempre (dev y producción) desde `server.ts` inmediatamente después de `connectDB()`: aplica de forma idempotente un subconjunto de cambios de esquema que `sync()` no cubre (columnas de `garment_types`, ENUMs de `store_orders.status`, columnas/índice de `tracking_token`, seed de `tracking_link_expiry_days`, `store_orders.customer_dni` + `store_customers.dni`). Tiene guardas explícitas para no reconstruir un ENUM de MySQL si el valor ya existe (un `ALTER` de ENUM bloquea la tabla).
...
| 099 | 2026-08-25 | **Ficha técnica + código interno en `catalog_products`**: agrega `internal_code` (texto libre, `unique: true`) y ~25 columnas que replican los campos de `order_items` (telas, colores, cuello/manga, marca/escudo, detalle de tela, sponsors, bordado, puño, accesorios) — todas nullable, porque acá el producto es una **plantilla**, no un ítem de pedido concreto. Al crear un pedido de producción desde un producto de catálogo (`CatalogPage.tsx`, botón "Pedido"), estos valores precargan el primer `OrderItem` del pedido nuevo (`NewOrderPage.tsx`, `prefillItem`) y quedan editables ahí. `internal_code` y toda esta ficha técnica quedan **excluidas** de la API pública de la tienda (`store.service.ts` — `listStoreProducts`/`getStoreProduct` ahora usan un `attributes` allowlist explícito, `PUBLIC_PRODUCT_ATTRIBUTES`) para no exponerlas sin auth. Ver [02-FUNCTIONAL-MAP.md](02-FUNCTIONAL-MAP.md) (Catálogo mayorista). |
| 101 | 2026-09-01 | **`store_coupons.link_url`** (VARCHAR 500, nullable): destino del botón "Ver la colección" del popup promocional de cupón (`StoreLayout` → `CouponPopup`). Antes iba fijo a `/tienda/productos`; ahora cada cupón apunta a la sección/producto que se publica. Texto libre, mismo resolver que `store_hero_image_link` en el front (`frontIndians/src/utils/links.ts` → `resolveStoreLink`). Vacío/null → fallback a `/tienda/productos`. Replicada en `ensureSchema.ts`. |
| 103 | 2026-09-04 | **Código de barras único en `catalog_products`**: columna `barcode` (VARCHAR 50, nullable) + índice único `uq_catalog_products_barcode` creado con `addIndex` explícito (no `unique: true` en el modelo, para no duplicarlo bajo `sync()` — mismo patrón que `cash_transactions.idempotency_key`). Backfill determinístico en la propia migración (`PRD-NNNNNN` a partir del `id`) para los productos ya existentes; los nuevos lo reciben en `catalog.service.ts` (`createProduct`) apenas se conoce el `id`. Replicada en `ensureSchema.ts`. Rama `feat/product-barcode`, sin mergear. |
| 102 | 2026-09-04 | **DNI obligatorio en el checkout**: `store_orders.customer_dni` y `store_customers.dni` (ambas VARCHAR 15, nullable). El DNI del pedido es el dato de despacho del envío (etiqueta + PDF); el del cliente es solo para autocompletar el próximo checkout. Las dos columnas son NULL-ables a nivel base (pedidos/cuentas previos no tienen DNI); la obligatoriedad se aplica en `checkoutValidators` (7–9 dígitos, se limpian puntos/espacios), no en el esquema. Replicada en `ensureSchema.ts`. Ver `BR-STORE-013`. Rama `feature/checkout-dni-obligatorio`, sin mergear. |
| 106 | 2026-09-08 | **`employees` + `employee_events`**: legajo del personal + histórico de novedades. `employees` = ficha (datos personales/laborales, `current_remuneration` DECIMAL(12,2) con getter, `sector` texto libre indexado, `active` + `termination_date` para baja lógica); DNI único `uq_employees_dni` por `addConstraint`. `employee_events` = novedades (ENUM `type` con 8 valores, `event_date` propia, `amount`/`previous_amount` para `salary_change`, `created_by_user_id` → autor); FK `employee_id` con `ON DELETE CASCADE`. Registro pensado como inmutable (sin endpoint de edición; solo `admin` borra). Módulo entero restringido a `admin`. Tablas nuevas y aisladas — solo referencian `users`. **No** replicadas en `ensureSchema.ts` (tablas nuevas → `sync()` las crea en dev). Ver [02-FUNCTIONAL-MAP.md](02-FUNCTIONAL-MAP.md) módulo 2c. |
| 105 | 2026-09-08 | **`suppliers`**: directorio autónomo de proveedores (telas, avíos, servicios). Tabla nueva y aislada — no referencia ni es referenciada por stock/costos/pedidos. Campos comerciales completos + `category` (rubro, texto libre, indexado) + `active` (baja lógica). `tax_id` normalizado a 11 dígitos con índice único `uq_suppliers_tax_id` creado por `addConstraint` (no `unique: true` en el modelo, para no duplicarlo bajo `sync()` — mismo patrón que `cash_transactions.idempotency_key`). Migración aditiva con guard `showAllTables()`. **No** está replicada en `ensureSchema.ts`: al ser una tabla nueva, `sync()` la crea entera en dev. Ver [02-FUNCTIONAL-MAP.md](02-FUNCTIONAL-MAP.md) módulo 2b. |
| 100 | 2026-08-26 | **`store_order_sequences`**: contador atómico del número de pedido de tienda (`ECOM-YYYYMMDD-NNNN`), hallado y corregido en el test de carga de `stress/`. Reemplaza el patrón anterior (`SELECT` último `order_number LIKE 'prefix%'` → +1 → `INSERT`), que bajo checkouts concurrentes calculaba el mismo número dos veces — el índice único de `order_number` evitaba el duplicado real, pero tiraba abajo uno de los dos checkouts con 409. Un primer intento de arreglo (agregar `lock: FOR UPDATE` a ese mismo `SELECT`) generó deadlocks reales de InnoDB (gap locks por ser una condición de rango). La tabla usa el modismo estándar de MySQL `INSERT ... ON DUPLICATE KEY UPDATE next_seq = LAST_INSERT_ID(next_seq + 1)` — una sola sentencia, lock de fila exacta por PK, sin la ventana de carrera ni el deadlock de rango. Ver `generateStoreOrderNumber` en `store.service.ts` (el comentario ahí documenta los dos intentos fallidos). **2026-09-07**: el segmento `YYYYMMDD` (y por lo tanto la clave `date_key` de esta tabla) se calculaba con `new Date().getDate()` — hora del servidor, que en producción es UTC. Un pedido hecho entre las 21:00 y la medianoche de Tucumán (UTC−3) quedaba numerado con la fecha del día siguiente y sumaba a la secuencia de esa otra jornada (caso real: `ECOM-20260905-0002` para un pedido del 2026-09-04 22:13). Corregido con `storeOrderDateKey()` → `businessDate()` (mismo helper y mismo defecto ya arreglados en el asiento de caja y en el filtro por fecha del listado admin). Test: `src/__tests__/unit/storeOrderDateKey.test.ts`. |
...
| Clientes | `clients` |
| Proveedores | `suppliers` (directorio autónomo, sin FKs a otros dominios — migración 105) |
| Empleados / RRHH | `employees`, `employee_events` (legajo + novedades; solo FK a `users` como autor — migración 106) |
