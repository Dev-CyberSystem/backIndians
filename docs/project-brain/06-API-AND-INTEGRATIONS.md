...
### `/employees` — `employee.routes.ts` — todo `authorize('admin')`
...
- **Cambio de contrato (2026-09-04)**: `POST /checkout` ahora **exige** `customerDni` (422 si falta o no son 7–9 dígitos tras limpiar puntos/espacios). Se guarda en `store_orders.customer_dni` y, para compradores con cuenta sin DNI, se copia a `store_customers.dni`. `PUT /store/me` acepta además un campo `dni` opcional con la misma validación. Backend y frontend deben desplegarse juntos. Ver `BR-STORE-013` y la migración `102`. Rama `feature/checkout-dni-obligatorio`, sin mergear.
...
- **Cambio aditivo (2026-09-07)**: `GET /store/settings` incluye además `order_expiry_hours` — las horas hasta la cancelación automática de un pedido impago (`BR-STORE-004`). Es una clave **derivada**, no una fila de `settings`: sale de la env `ORDER_EXPIRY_HOURS` y está declarada en `PUBLIC_DERIVED_SETTING_KEYS`. La tienda la usa para advertir el plazo real en vez de hardcodear "48". Ver `BR-STORE-014`.
...
- **Acreditación**: el resultado de un pago se aplica en un único lugar por circuito — `applyPaymentResult` (tienda, `store.service.ts`) y `applyCatalogPaymentResult` (catálogo, `catalog.service.ts`), compartidos por el webhook y por el job. Ambos validan moneda contra ARS antes de acreditar, son idempotentes y nunca acreditan a ciegas. `applyPaymentResult` además es **anti-retroceso**: si el pedido ya salió de `pending_payment` (el admin lo movió a `processing`/`shipped`/etc.), un webhook o una confirmación repetidos con el mismo pago `approved` NO lo devuelven a `paid` ni a `pending_payment` — solo refrescan los metadatos de MP y loguean `store.webhook.staleForwardIgnored` (fix `fix/store-order-status-revert-pagado`, 2026-09-02; antes esto revivía el pedido a "Pagado" solo). En catálogo el cobro se materializa como una fila de `catalog_invoice_payments` con `payment_method='mercadopago'` e `idempotency_key = mp-<paymentId>` (índice único, migración 094), que actualiza `payment_amount`/`status` de la factura y genera el asiento de caja (DEC-012).
