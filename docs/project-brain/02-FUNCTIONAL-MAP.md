...
**Sección de campaña "El Pulga" (2026-08-27)**: landing propia en `/tienda/el-pulga` (homenaje a Luis Miguel Rodríguez, "Despedida Monumental PR7"). No es un módulo nuevo ni toca el esquema: es una página de frontend (`frontIndians/src/pages/store/StorePulgaPage.tsx`) que lista los productos de catálogo con un `tag` configurable (default "El Pulga") vía el filtro `tag` que ya existe en `listStoreProducts`. Las camisetas son productos de catálogo normales (`show_in_store=true`) — siguen visibles en el catálogo general, la sección solo les da un espacio de campaña. Todo el contenido (imagen de fondo desktop/mobile, título, bajada, descripción, CTA, link de Instagram) se edita en *Tienda online → Configuración → Sección El Pulga* mediante claves `store_pulga_*` de la tabla `settings` (agregadas a `VALID_KEYS` y `PUBLIC_SETTING_KEYS` en `settings.service.ts`, sin migración). Con `store_pulga_enabled != 'true'` la ruta redirige a `/tienda` y el ítem desaparece del menú, del footer y de la franja de la home.
