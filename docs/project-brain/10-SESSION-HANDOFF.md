...
Producción: dos certificados `IndiansProd` válidos y coincidentes con la clave, vigencia hasta 14/09/2028. Se eligió el más reciente y se cargó solo en `.env` local como `_PROD`. Tras relacionar `IndiansProd` con `wsfe`, ambos autenticaron en WSAA producción. WSFE confirmó Factura C, Consumidor Final y PV 3 activo/no bloqueado; último número de Factura C: 0. Fueron únicamente consultas de lectura: no se pidió CAE ni se modificó producción. Configuración local actual: datos fiscales definitivos, `afip_environment=homo`, `afip_enabled=false`.

Homologación real aprobada: Factura C PV 3 número 1 por $121 y Nota de Crédito C PV 3 número 1 total, ambas con CAE y vencimiento 24/09/2026. Idempotencia de NC aprobada. PDF de ambos comprobantes renderizado e inspeccionado. `FEParamGetPtosVenta` devuelve 602 vacío pero `FECompUltimoAutorizado` acepta PV 3; se agregó excepción estricta solo para homo, producción sigue fail-closed. Suite ARCA final 26/26 y typecheck aprobados. No se emitió ni configuró producción.

Ensayo de release `v1.11.2` aprobado en la rama: backend typecheck y suite completa 64/64 (497/497 tests); frontend 52/52, build y prerender 38/38. Al ser dry-run no creó commit, tag, backup ni despliegue. Para publicar falta autorización explícita del release productivo: secretos `_PROD` en Railway, migración 108, configuración fiscal con `afip_enabled=false`, despliegue conjunto y smoke. La primera emisión productiva requiere una autorización posterior y separada.
